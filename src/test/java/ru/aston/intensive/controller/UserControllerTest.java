...
class UserControllerTest {
    ...
    void findAll_whenOk() throws Exception {
        List<UserEntity> userEntityList = List.of(
                new UserEntity("Vasua", "vasua@test.org", 30),
                new UserEntity("Petua", "petya@petya.org", 22)
        );
    ...
    void create_whenOk() throws Exception {
        UserDto notExistUser = getNotExistUser();
        ...
        mockMvc.perform(post(URI_START)
                ...
                .andExpect(jsonPath("$.name").value(notExistUser.getName()))
                .andExpect(jsonPath("$.email").value(notExistUser.getEmail()))
                .andExpect(jsonPath("$.age").value(notExistUser.getAge()));
    ...
    void create_whenEmailIsExist_status409Conflict() throws Exception {
        ...
        BDDMockito.given(userService.create(any(UserEntity.class)))
                .willThrow(new EmailExistingException("User already exist with email = " + notExistUser.getEmail()));
    ...
    void update_whenOk() throws Exception {
        ...
        UserDto sendUser = UserMapper.entityToDto(updatedUser);
        ...
        mockMvc.perform(put(URI_START + updatedUser.getId())
                ...
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.name").value(sendUser.getName()))
                .andExpect(jsonPath("$.email").value(sendUser.getEmail()))
                .andExpect(jsonPath("$.age").value(sendUser.getAge()));
    ...
    void update_whenSendUserIncorrect_status400BadRequestAndErrors() throws Exception {
        ...
        UserDto sendUser = UserMapper.entityToDto(invalidUser);
    ...
    private UserDto getNotExistUser() {
        return new UserDto("Anya", "anya@anya.org", 80);
    }
}
